...
const ServerStatus = () => {
  ...
  const getStatusText = (status) => {
    switch (status) {
      ...
      case 'error':
        return t('serverStatus.error');
      case 'checking':
        return t('serverStatus.checking');
      default:
        return t('serverStatus.unknown');
    }
  };
    ...
    <div className="bg-white rounded-lg shadow p-6 mb-6">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-4">
          ...
          <span className="ml-2 text-gray-600">{t('serverStatus.checkingServer')}</span>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div className="flex items-center">
              ...
              <div>
                <p className="font-medium text-gray-800">{t('serverStatus.trytonServer')}</p>
          ...
          {serverStatus.tryton === 'error' && errorDetails && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <div className="flex">
                ...
                <div>
                  <p className="text-sm text-red-800 font-medium">{t('serverStatus.serverError')}</p>
          ...
          {serverStatus.tryton === 'stopped' && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div className="flex">
                ...
                <div>
                  <p className="text-sm text-yellow-800 font-medium">{t('serverStatus.serverNotRunning')}</p>
                  <p className="text-sm text-yellow-700 mt-1">
                    {t('serverStatus.solutionCommand')}<code className="bg-yellow-100 px-1 rounded">gnuhealth-control start</code>
                  </p>
                </div>
              </div>
            </div>
          ...
          {serverStatus.tryton === 'error' && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex">
                ...
                <div>
                  <p className="text-sm text-blue-800 font-medium">{t('serverStatus.serverInternalProblems')}</p>
                  <p className="text-sm text-blue-700 mt-1">
                    {t('serverStatus.serverInternalProblemsDesc')}
                  </p>
                </div>
              </div>
            </div>
